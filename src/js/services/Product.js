...
export default class Product {
  randomData() {
    const stateRandom = Math.floor(
      Math.random() * Object.values(VALID_STATES).length
    );
    ...
    const fruitRandom = Math.floor(
      Math.random() * Object.values(VALID_FRUITS).length
    );
  ...
  async get(id) {
    ...
    const row = result.data.rows.find((r) => {
      return r.key === id;
    });
    return row.doc;
  }
